...
class DataValidator:
    ...
    def validate_image_filename(filename: str) -> bool:
        """Valida que el nombre de archivo tenga una extensión de imagen válida."""
        if not isinstance(filename, str):
            return False
        return any(filename.lower().endswith(ext) for ext in DataValidator.IMAGE_EXTENSIONS)

    @staticmethod
    def validate_image_data(image_data: bytes) -> bool:
        """Valida que los datos de la imagen sean válidos y correspondan a un formato soportado."""
        try:
            img = PILImage.open(BytesIO(image_data))
            return img.format.lower() in DataValidator.SUPPORTED_IMAGE_FORMATS
        except Exception:
            return False

    @staticmethod
    def convert_to_png(image_data: bytes) -> tuple[bytes, str]:
    ...
    def validate_fields(ctx: Context, key_figures: List, key_values: Dict, start_date: Optional[str] = None, end_date: Optional[str] = None, specific_dates: Optional[List[str]] = None, tool_name: str = None) -> Dict:
        ...
        if start_date and end_date:
            ...
            raise ValueError("Both start_date and end_date required for range queries")
