...
def analyze_compliance(
    ...
    try:
        ...
        # Determinar el campo identificador desde config.yaml
        identifier_fields = config["pdf_extraction"].get("identifier_fields", ["machine"])
        pdf_name_template = config["pdf_extraction"].get("pdf_name_template", "{identifier}.pdf")
        ...
        # Construir identifiers desde los datos MES
        identifiers = set()
        ...
        try:
            ...
            for obj in objects:
                if obj.object_name.endswith(".pdf"):
                    ...
                    identifier = filename.replace(".pdf", "")
                    if identifier in identifiers or not identifiers:  # Cargar solo PDFs relevantes
                        if identifier in sop_rules_cache and sop_rules_cache[identifier]["timestamp"] + CACHE_TTL > time.time():
                            sop_content[identifier] = sop_rules_cache[identifier]["content"]
                            logger.info(f"Contenido para identifier={identifier} recuperado de la caché")
                        else:
                            pdf_result = json.loads(minio_client.get_pdf_content(filename))
                            if pdf_result["status"] == "success":
                                ...
                                analysis_notes.append(f"Failed to load SOP for identifier={identifier}: {pdf_result['message']}")
                                logger.warning(f"Failed to load SOP for identifier={identifier}: {pdf_result['message']}")
        except Exception as e:
        ...
        for record in fetch_result["data"]:
            ...
            analysis = {
                ...
                **{k: record.get(k) for k in key_values},
                ...
                "sop_content": sop_content.get(identifier, "") if identifier else ""
            }
